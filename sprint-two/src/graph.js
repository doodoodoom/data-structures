...
g.hasEdge(4, 3) first checks for fromNode, and see if its edges property includes toNode
Then checks for toNode, and see if its edges property includes fromNode
 */
...
Graph.prototype.removeEdge = function(fromNode, toNode) {
  for (var count in this) {
    if (typeof this[count] === "object") {
      ...
      if (singleNode.value === toNode) {
        for (var i = 0; i < singleNode.edges.length; i++) {
          if (singleNode.edges[i] === fromNode) {
            singleNode.edges.splice(i, 1);
          }
        }
      }
